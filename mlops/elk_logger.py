...
_client = None  # Elasticsearch client instance (None until first connection attempt)
_es_unavailable = False  # cached flag to skip retries once ES is confirmed down

def _get_client():
    global _client, _es_unavailable
    if not _ES_AVAILABLE or _es_unavailable:
        return None
    if _client is None:
        try:
            _client = Elasticsearch(ES_HOST, request_timeout=0.5)
            if not _client.ping():
                log.warning("Elasticsearch not reachable at %s -- skipping ES logging", ES_HOST)
                _es_unavailable = True
                _client = None
        except Exception as e:
            log.warning("Failed to connect to Elasticsearch: %s -- disabling ES logging", e)
            _es_unavailable = True
    return _client
...
def log_prediction(inputs: dict[str, Any], outputs: dict[str, Any]) -> None:
    ...
    doc = {
        ...
        # City simulator UE fields (passed as extra inputs)
        "master_id": inputs.get("master_id"),
        "scenario":  inputs.get("scenario"),
        "ue_lat":    inputs.get("ue_lat"),
        "ue_lng":    inputs.get("ue_lng"),
    }
